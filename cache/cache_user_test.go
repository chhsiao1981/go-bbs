...
func TestAddToUHash(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			userID, err := GetUserID(tt.args.uidInCache + 1)
			if err != nil {
...
func TestRemoveFromUHash(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		assert.Equal(t, int32(i+1), nextInHash[i])
	}
	assert.Equal(t, int32(-1), nextInHash[4])
	for i := 5; i < len(nextInHash); i++ {
		assert.Equal(t, int32(0), nextInHash[i])
	}

	nextInHash1 := &[ptttype.MAX_USERS]int32{}
	copy(nextInHash1[:], []int32{1, 2, 3, 4, -1})

	nextInHash2 := &[ptttype.MAX_USERS]int32{}
	copy(nextInHash2[:], []int32{1, 3, 3, 4, -1})

	nextInHash3 := &[ptttype.MAX_USERS]int32{}
	copy(nextInHash3[:], []int32{1, 3, 3, -1, -1})
