...
type BoardHeaderRaw struct {
	...
	Next               [BSORT_BY_MAX]int32 /* 在同一個gid下一個看板 動態產生*/
	FirstChild         [BSORT_BY_MAX]int32 /* 屬於這個看板的第一個子看板 */
...
const BOARD_HEADER_TITLE_OFFSET = unsafe.Offsetof(emptyBoardHeaderRaw.Title)
const BOARD_HEADER_BRD_ATTR_OFFSET = unsafe.Offsetof(emptyBoardHeaderRaw.BrdAttr)
const BOARD_HEADER_FIRST_CHILD_OFFSET = unsafe.Offsetof(emptyBoardHeaderRaw.FirstChild)
